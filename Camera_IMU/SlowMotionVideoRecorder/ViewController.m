...
- (void)loadState {
    ...
    if (state == 0) {
        ...
        // Take IMU video
        self.statusLabel.text = @"Step 4: Record IMU Video";
...
-(void)nextStep {
    if (self.sequences.currentStep < 3) {
        self.lastBtn.enabled = false;
        self.nextBtn.enabled = false;
...
- (void)didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL error:(NSError *)error {
    ...
    if (state == 1) {
        [self.sequences setScanForActiveSequence:outputFileURL];
    } else if (state == 3) {
...
-(void)didFinishSavingImageAtURL:(NSURL *)outputFileURL error:(NSError *)error {
    ...
    if (state == 0) {
        ...
        [self.sequences setScaleQRForActiveSequence:outputFileURL];
    }
